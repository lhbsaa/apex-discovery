...
### Added
- **Full engine port**: All 10 core engines, 38 development skills, 18 agents, 6 modes, 7 phases fully integrated from the predecessor engine
- **149 scientific skills**: All skills from scientific-agent-skills embedded directly in `skills/` directory
- **Standalone architecture**: No external dependencies, no git clone needed
...
### Changed
- **Architecture**: From extension wrapper → fully standalone project
- **`cli/main.js`**: Rewritten from thin wrapper (72 lines) to full CLI (515 lines) with all 12 commands
- **`config/defaults.json`**: Merged 6 modes total (research-scientist + 5 dev modes)
...
### Fixed
- CI would fail because test asserted a sibling directory path
...
- Double-counting of skills in status output
...
- 3 core tests (skills dir, config, apex engine detection)
